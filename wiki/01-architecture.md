...
### Frontend — `src/`
A Vite + React 19 single-page app (no TypeScript, no state-management library beyond React context). Entry point `src/main.jsx` mounts `<App/>` in `StrictMode`. `src/App.jsx` wraps the app in `AuthProvider` (`src/auth/AuthContext.jsx`) and a `react-router-dom` v7 `BrowserRouter` with three routes: `/` (`src/pages/HomePage.jsx`), `/login` (`src/pages/LoginPage.jsx`), and `/chat` (`src/pages/ChatPage.jsx`, gated by `src/components/ProtectedRoute.jsx`). Data access is split between `src/api/client.js` (auth-header injection, 401 handling) and `src/repositories/*` (domain calls for auth, conversations, and documents). Built by `Dockerfile` (multi-stage `node:22-alpine` build → static files served by `nginx:alpine`).

### Document preview — `src/components/FilePreview.jsx`, `PageImageViewer.jsx`, `src/hooks/usePageImages.js`
PDFs are previewed as **server-rendered page images**, not parsed in the browser. `FilePreview` branches on `document.mime_type`: images are fetched whole and shown directly; PDFs delegate to `PageImageViewer` (keyed on document id, so switching documents mounts a fresh instance) and the source PDF is never downloaded; anything else degrades to a download link.

`usePageImages` fetches the page manifest (`GET /api/documents/{id}/pages` → `[{page, width, height}]`) up front and then individual page images on demand, driven by an `IntersectionObserver` with a 600px preload margin plus an eager fetch of any cited page. Images go through `apiFetch` into object URLs rather than a bare `<img src>`, because the route is authenticated and an `<img>` cannot send the `Authorization` header; every URL is revoked on unmount. Each page wrapper takes its `aspect-ratio` from the manifest so scroll height is correct before bytes arrive — which is what makes scroll-to-citation land accurately. `CitationOverlay` positions normalized rects over that wrapper unchanged, since a rendered page is geometrically similar to the source page rect at any DPI.

This replaced a `pdfjs-dist` canvas viewer (~36MB dependency, client-side rasterization of every page).
...
Upload is synchronous (validate → save file → insert `Document` row with `status="pending"`), then ingestion is asynchronous via a single Celery task, `ingest_document` (`backend/app/workers/tasks.py`), brokered through Redis (`backend/app/workers/celery_app.py`). The task parses (PyMuPDF for PDFs, python-docx for docx, direct OCR for images), rasterizes PDF pages for preview (`backend/app/services/page_images.py` → `uploads/pages/{document_id}/{page:04d}.webp`, non-fatal), chunks (parent ~1500 tokens / child ~300 tokens, via `langchain-text-splitters`), embeds children (OpenRouter embeddings), and writes `DocumentParentChunk`/`DocumentChunk` rows. Clients poll ingestion progress via an SSE endpoint that reads the `documents.status` column every 2s.
